...
import static com.rollerite.message.RolleriteMessage.COMMAND_ENDERCHEST_USAGE;
import static com.rollerite.message.RolleriteMessage.PLAYER_NOT_FOUND;

@RequiredArgsConstructor
public class EnderchestCommand implements BasicCommand
{
	...
	public void execute(CommandSourceStack commandSourceStack, String[] args)
	{
		...
		if(args.length == 0 || args.length == 1)
		{
			...
			Player target = args.length == 0 ? player : rolleritePlugin.getServer().getPlayerExact(args[0]);
		...
		{
			rolleritePlayer.sendMessage(COMMAND_ENDERCHEST_USAGE);
		}
	}
